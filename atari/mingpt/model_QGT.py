...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        total_tokens = 3 * config.block_size + config.upper_bound_dim
...
class DecisionTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.pos_emb = nn.Parameter(torch.zeros(1, 3*config.block_size + config.upper_bound_dim, config.n_embd))
...
## original 
    ...
    def forward(self, mask_length, rtgs,  query_results, upper_bounds, queries=None,targets=None):
        ...
        triplet_mask = torch.zeros((query_results.shape[0], query_results.shape[1]*3 , 1), dtype=torch.int, device=query_result_embeddings.device)
        ...
        if queries is not None : 
            ...
            token_embeddings[:,1::3,:] = query_result_embeddings
        ...
        print(upper_bounds_embedding.shape)
        ...
        triplet_mask = torch.cat([ub_mask, triplet_mask], dim=1)  # shape becomes (B, T+ub_len)
